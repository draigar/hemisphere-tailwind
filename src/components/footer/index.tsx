...
import { footerType } from '@web/types';
import { useQueries } from '@web/hooks/useQueries';
...
const Footer = (props: FooterProps) => {
    const { footer } = props;
    const { fetchRefByData } = useQueries();
    const [footerItems, setFooterItems]: any = React.useState({});
    ...
    React.useEffect(() => {
        if (footer._ref) {
            ...
            setFooterItems(fetchRefByData.data)
        }
    }, [footer._ref])
    ...
    const TopFootSection = () => (
        <div className="flex justify-center items-center lg:justify-between p-6 border-b border-gray-300">
            <div className="mr-12 hidden lg:block">
                <span>{fetchRefByData?.data?.topSectionText}</span>
            </div>
    ...
    const BottomFootSection = () => (
        <div className="flex justify-between">
            <div className="p-6">
                <span>{fetchRefByData?.data?.bottomSection?.copytextLeft}</span>
            </div>
            <div className="p-6">
                <span>{fetchRefByData?.data?.bottomSection?.copytextRight}</span>
            </div>
        </div>
    );
        ...
        <div>
            ...
            <div className="mx-6 py-10 text-center md:text-left">
                <div className={`grid grid-1 md:grid-cols-2 ${fetchRefByData?.data?.footerType === '4rows' ? 'lg:grid-cols-4' : 'lg:grid-cols-3' } gap-8`}>
                    ...
                    <div className="">
                        <CustomText content={fetchRefByData?.data?.row2} />
                    </div>
                    <div className="">
                        <CustomText content={fetchRefByData?.data?.row3} />
                    </div>
                    {fetchRefByData?.data?.footerType === '4rows' && <div className="">
                        <CustomText content={fetchRefByData?.data?.row4} />
                    </div>}
                </div>
            </div>
            {fetchRefByData?.data?.bottomFooterSection && (<BottomFootSection />)}
        </div>
    );
